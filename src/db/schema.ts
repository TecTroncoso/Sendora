...
async function getSchemaVersion(): Promise<number> {
  ...
  try {
    ...
    // Tabla no existe todavía — primera ejecución
  }
  return 0;
}
...
export async function initDatabase(): Promise<void> {
  ...
  if (currentVersion >= CURRENT_SCHEMA_VERSION) {
    console.log(`✅ Base de datos OK (v${currentVersion})`);
    return;
  }

  console.log(`📦 Inicializando base de datos (v${currentVersion} → v${CURRENT_SCHEMA_VERSION})...`);

  // V1: Esquema base + migración mixed content
  if (currentVersion < 1) {
    await db.batch(
      [
        // Version tracking
        ...
        // Contenido programado (ya con soporte 'mixed')
  ...
  // V2: Legacy migration fixes (scheduled_content mixed + send_log FK repair)
  if (currentVersion < 2) {
    try {
  ...
  // Asegurar que la tabla existe (podría no existir si venimos de una DB pre-versionada)
  ...
  console.log(`✅ Base de datos (multi-usuario) inicializada correctamente (v${CURRENT_SCHEMA_VERSION})`);
}
